...
const HTML = {
	BTN_INNER:
		'<div id="accManagerBtn" class="button buttonB bigShadowT" onmouseenter="playTick()" style="display:block;width:300px;text-align:center;padding:15px;font-size:23px;pointer-events:all;padding-bottom:22px;margin-left: 4px;margin-top:5px">Alt-Manager</div>',
	ALT_MENU:
		'<div id="altAccounts"></div><div id="buttons"><div class="accountButton" id="altAdd">Add new account</div></div>',
...
class AccountManager {
	...
	constructor() {
		...
		!localStorage.getItem("altAccounts") &&
			localStorage.setItem("altAccounts", "[]");
	}
	...
	#addAccount = (name, pass) => {
		if (name.replace(/\s/, "") === "" || pass.replace(/\s/, "") === "")
			return alert("Username and Password fields must not be empty.");
		let users = JSON.parse(localStorage.getItem("altAccounts"));
		if (users.find((e) => e.username === name))
			return alert("This Username has already been added.");
	...
	#deleteAccount = (name) => {
		localStorage.setItem(
			"altAccounts",
			JSON.stringify(
				JSON.parse(localStorage.getItem("altAccounts")).filter(
					(e) => e.username !== name
				)
			)
		);
	...
	#login = (name, pass) => {
		...
		/** @type {inputs} */ (
			document.getElementById("accPass")
		).style.display = "none";
		/** @type {inputs} */ (
			document.getElementsByClassName("accountButton")[0]
		).style.display = "none";
		/** @type {inputs} */ (
			document.getElementsByClassName("accountButton")[1]
		).style.display = "none";
	};
	...
	#watcher = () => {
		...
		document.getElementById("altAdd").addEventListener("click", () => {
			...
			this.addWin.show();
			document
				.getElementById("addAccountButtonB")
		...
		document.querySelectorAll(".altlistelement").forEach((i) =>
			i.addEventListener("click", (e) => {
				let selected = storage.find(
					(obj) =>
						obj.username ===
						/** @type {inputs} */ (e.target).innerText
				);
				this.#login(
					selected.username,
					!!selected.format && selected.format === "b64"
						? Buffer.from(
								String(selected.password),
								"base64"
						  ).toString("ascii")
						: selected.password
				);
		...
		document.querySelectorAll(".altdeletebtn").forEach((i) =>
			i.addEventListener("click", (e) => {
				// @ts-ignore
				let tar = e.target.previousElementSibling.innerText;
				confirm(
					`Do you really want to remove the account "${tar}" from the Alt-Manager?`
				) && this.#deleteAccount(tar);
			})
		);
	};
